...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	public void startGame() {
		...
		String instructions = "Your snake cannot stop moving to collect food.  Try to get you snake to grow longer by collecting food.  But make sure you don't run into the walls or even yourself!";
	...
	private void setFoodLocation() {
		...
		Location l = new Location(L1.nextInt(WIDTH), L2.nextInt(HEIGHT));
		...
		if(!snake.isLocationOnSnake(l)) {
			...
			l = new Location(L1.nextInt(WIDTH), L2.nextInt(HEIGHT));
		}
		}
	}
	...
	private void gameOver() {
		...
		int b = JOptionPane.showConfirmDialog(null, "Do you want to play again?");
		...
		if(b==0) {
			...
			snake.reset(loc);
			setFoodLocation();
			timer.start(); //if it doesn't work try restart
		}
	}
